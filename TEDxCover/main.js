    ...
    (new (function() {
        ...
        (i = [{
            ...
            value: function() {
                ...
                this.geometries = [new r, new s, new a],
            ...
            key: "addAmbientLight",
            value: function() {
                var t = new THREE.AmbientLight("#2900af",1);
                this.scene.add(t)
            }
        }, {
            key: "addSpotLight",
            value: function() {
                var t = new THREE.SpotLight("#e000ff",1,1e3);
                t.position.set(0, 27, 0),
                t.castShadow = !0,
                this.scene.add(t)
            }
        }, {
            key: "addRectLight",
            value: function() {
                var t = new THREE.RectAreaLight("#ff0000",1,2e3,2e3);
                t.position.set(5, 50, 50),
                t.lookAt(0, 0, 0),
                this.scene.add(t)
            }
        }, {
                ...
                for (var t = new THREE.MeshPhysicalMaterial({
                    ...
                    for (var i = 0; i < 1; i++)
                        for (var n = this.getTotalRows(e), o = 0; o < n; o++) {
                            ...
                            s.position.y = 0,
                            s.position.x = o + o * this.gutter.size + (n === this.grid.cols ? 0 : 2.5),
                            s.position.z = e + e * (i + .25),
                            s.rotation.x = r.rotationX,
                            s.rotation.y = r.rotationY,
                            s.rotation.z = r.rotationZ,
                            s.initialRotation = {
                                x: s.rotation.x,
                                y: s.rotation.y,
                                z: s.rotation.z
                            },
                            this.groupMesh.add(s),
                            this.meshes[e][o] = s
                        }
                }
                var a = -this.grid.cols / 2 * this.gutter.size - 1
                  , h = -this.grid.rows / 2 - .8;
                this.groupMesh.position.set(a, 0, h),
                        ...
                        for (var u = 0; u < 1; u++)
                            for (var l = this.getTotalRows(d), f = 0; f < l; f++) {
                                var w = this.meshes[d][f]
                                  , y = (t = h,
                                e = c,
                                i = w.position.x + this.groupMesh.position.x,
                                r = w.position.z + this.groupMesh.position.z,
                                Math.sqrt(Math.pow(t - i, 2) + Math.pow(e - r, 2)))
                                  , p = o(y, 7, 0, 0, 6);
